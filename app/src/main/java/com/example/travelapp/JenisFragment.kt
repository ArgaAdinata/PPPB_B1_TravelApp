...
class JenisFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding){
            ...
            adapterTicket.setDropDownViewResource(androidx.appcompat.R.layout.support_simple_spinner_dropdown_item)
            spinnerTicket.adapter = adapterTicket

            btnBuy.setOnClickListener {
                findNavController().apply {
                    previousBackStackEntry?.savedStateHandle?.set("jenis",
                        spinnerTicket.selectedItem.toString())
                }.navigateUp()
            }
        }
    }
